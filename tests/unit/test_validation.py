...
from cleanepi.core.config import CleaningConfig, NumericConfig, SubjectIDConfig
...
class TestValidateDataFrame:
    ...
    def test_minimum_rows_validation(self):
        ...
        with pytest.raises(
            ValueError, match="DataFrame has 1 rows, minimum 2 required"
        ):
    ...
    def test_minimum_columns_validation(self):
        ...
        with pytest.raises(
            ValueError, match="DataFrame has 1 columns, minimum 2 required"
        ):
...
class TestValidateConfig:
    ...
    def test_subject_id_config_without_target_columns(self):
        ...
        with pytest.raises(
            ValueError, match="standardize_subject_ids requires target_columns"
        ):
            validate_config(config)
...
def sample_config():
    ...
    return CleaningConfig(verbose=True, strict_validation=False)
