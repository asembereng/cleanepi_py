...
class TestStandardizeColumnNames:
    ...
    def test_keep_columns(self):
        ...
        df = pd.DataFrame(
            {"Keep Me": [1, 2, 3], "Change Me": [4, 5, 6], "Also Keep": [7, 8, 9]}
        )
    ...
    def test_custom_rename(self):
        ...
        df = pd.DataFrame(
            {"Old Name 1": [1, 2, 3], "Old Name 2": [4, 5, 6], "Keep Same": [7, 8, 9]}
        )

        rename_map = {"Old Name 1": "new_name_1", "Old Name 2": "custom_column"}
    ...
    def test_keep_special_chars(self):
        ...
        df = pd.DataFrame({"Name@Symbol": [1, 2, 3], "Normal Name": [4, 5, 6]})

        result = standardize_column_names(df, remove_special_chars=False)

        # Special characters should be preserved
        assert any("@" in col for col in result.columns) or any(
            "symbol" in col.lower() for col in result.columns
        )

    def test_max_length(self):
        """Test maximum length constraint."""
        df = pd.DataFrame(
            {"Very Long Column Name That Exceeds Limit": [1, 2, 3], "Short": [4, 5, 6]}
        )
    ...
    def test_duplicate_column_names(self):
        ...
        assert "column_name_1" in columns or any(
            "column_name" in col for col in columns
        )
...
class TestStandardizeSingleName:
    ...
    def test_max_length_constraint(self):
        """Test maximum length constraint."""
        result = _standardize_single_name(
            "Very Long Column Name", "snake_case", True, 10
        )
...
class TestSuggestColumnRenames:
    ...
    def test_id_patterns(self):
        ...
        df = pd.DataFrame(
            {"Patient ID": [1, 2, 3], "Subject ID": [4, 5, 6], "Study ID": [7, 8, 9]}
        )
    ...
    def test_no_suggestions(self):
        ...
        df = pd.DataFrame(
            {"random_column": [1, 2, 3], "another_col": [4, 5, 6], "xyz": [7, 8, 9]}
        )
    ...
    def test_case_insensitive_matching(self):
        ...
        df = pd.DataFrame(
            {"PATIENT ID": [1, 2, 3], "patient id": [4, 5, 6], "Patient_ID": [7, 8, 9]}
        )
...
class TestEdgeCases:
    ...
    def test_unicode_characters(self):
        ...
        df = pd.DataFrame(
            {"Naïve Column": [1, 2, 3], "Café Data": [4, 5, 6], "数据列": [7, 8, 9]}
        )
    ...
    def test_numeric_column_names(self):
        ...
        df = pd.DataFrame({123: [1, 2, 3], 456.78: [4, 5, 6], 0: [7, 8, 9]})
