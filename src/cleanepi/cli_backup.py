...
Provides a simple CLI for data cleaning operations and service management.
"""
...
from cleanepi.service.manager import ServiceManager
from cleanepi.service.web_server import WebServerManager
...
def add_service_commands(subparsers):
    ...
    service_parser = subparsers.add_parser(
        "service", 
        help="Manage CleanEPI web service"
    )
    service_subparsers = service_parser.add_subparsers(dest="service_command", help="Service commands")
    
    # Start web server
    start_parser = service_subparsers.add_parser("start", help="Start web server")
    ...
    # Install service
    install_parser = service_subparsers.add_parser("install", help="Install as system service")
    install_parser.add_argument("--host", default="127.0.0.1", help="Host to bind to")
    install_parser.add_argument("--port", type=int, default=8000, help="Port to bind to")
    install_parser.add_argument("--no-autostart", action="store_true", help="Don't start automatically")
    ...
    # Open browser
    browser_parser = service_subparsers.add_parser("browser", help="Open web interface in browser")
    browser_parser.add_argument("--host", default="127.0.0.1", help="Host address")
    browser_parser.add_argument("--port", type=int, default=8000, help="Port number")
...
def handle_service_commands(args):
    ...
    if args.service_command == "start":
        ...
        if success:
            ...
            print(f"   Status: {'Started' if not args.no_autostart else 'Stopped'}")
            print("\nService management commands:")
            print("  cleanepi service status    - Check service status")
            print("  cleanepi service stop      - Stop the service")
            print("  cleanepi service restart   - Restart the service")
            print("  cleanepi service browser   - Open web interface")
        else:
    ...
    elif args.service_command == "browser":
...
def add_clean_commands(subparsers):
    """Add data cleaning commands."""
    clean_parser = subparsers.add_parser(
        "clean",
        help="Clean and standardize data files"
    )
    
    # Input/Output
    clean_parser.add_argument("input", help="Input data file (CSV, Excel, Parquet)")
