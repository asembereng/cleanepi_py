...
def clean_data(
    data: pd.DataFrame, config: Optional[CleaningConfig] = None, **kwargs
...
def _standardize_column_names(
    data: pd.DataFrame, config: Union[bool, dict]
    ...
    try:
        ...
        result = OperationResult(
            ...
            details={
                ...
                "renamed_count": sum(
                    1
                    for old, new in zip(data.columns, cleaned_data.columns)
                    if old != new
                ),
            },
        )
...
def _remove_constants(
    ...
    try:
        cleaned_data = remove_constants(
            data, cutoff=config.cutoff, exclude_columns=config.exclude_columns
        )
        ...
        result = OperationResult(
            ...
            columns_before=initial_shape[1],
            columns_after=cleaned_data.shape[1],
            details={"removed_columns": list(removed_columns), "cutoff": config.cutoff},
        )
...
def _convert_to_numeric(
    ...
    try:
        ...
        result = OperationResult(
            ...
            columns_before=initial_shape[1],
            columns_after=cleaned_data.shape[1],
            details={"target_columns": config.target_columns, "lang": config.lang},
        )
...
def _clean_using_dictionary(
    data: pd.DataFrame, dictionary: dict
) -> Tuple[pd.DataFrame, OperationResult]:
    ...
    try:
        ...
        result = OperationResult(
            ...
            details={
                ...
                "total_mappings": sum(
                    len(mappings) for mappings in dictionary.values()
                ),
            },
        )
...
def _check_date_sequence(
    data: pd.DataFrame, target_columns: list
) -> Tuple[pd.DataFrame, OperationResult]:
    ...
    try:
        ...
        result = OperationResult(
            ...
            columns_before=initial_shape[1],
            columns_after=cleaned_data.shape[1],
            details={"target_columns": target_columns},
        )
