...
class DateConfig(BaseModel):
    ...
    formats: Optional[List[str]] = Field(
        None,
        description="Expected date formats. If None, use intelligent parsing"
    )
    ...
    orders: Optional[Dict[str, List[str]]] = Field(
        None,
        description="Custom date parsing orders by category"
    )
...
class SubjectIDConfig(BaseModel):
    ...
    prefix: Optional[str] = Field(
        None,
        description="Expected prefix for subject IDs"
    )
    suffix: Optional[str] = Field(
        None,
        description="Expected suffix for subject IDs"
    )
    range: Optional[Tuple[int, int]] = Field(
        None,
        description="Valid numeric range for ID numbers as (min, max)"
    )
    nchar: Optional[int] = Field(
        None,
        ge=1,
        description="Expected total character length"
    )
...
class MissingValueConfig(BaseModel):
    ...
    target_columns: Optional[List[str]] = Field(
        None,
        description="Columns to process. If None, process all columns"
    )
    ...
    custom_na_by_column: Optional[Dict[str, List[str]]] = Field(
        None,
        description="Column-specific missing value strings"
    )
...
class DuplicateConfig(BaseModel):
    ...
    target_columns: Optional[List[str]] = Field(
        None,
        description="Columns to consider for duplicates. If None, use all columns"
    )
    subset: Optional[List[str]] = Field(
        None,
        description="Alias for target_columns for pandas compatibility"
    )
    keep: str = Field(
        "first",
...
class ConstantConfig(BaseModel):
    ...
    exclude_columns: Optional[List[str]] = Field(
        None,
        description="Columns to exclude from constant checking"
    )
...
class NumericConfig(BaseModel):
    ...
    target_columns: List[str] = Field(
        ...,
        description="Columns to convert to numeric"
    )
    lang: str = Field(
        "en",
        description="Language for number word conversion (e.g., 'one' -> 1)"
    )
    errors: str = Field(
        "coerce",
...
class CleaningConfig(BaseModel):
    ...
    standardize_column_names: Union[bool, Dict[str, Any]] = Field(
        True,
        description="Whether to standardize column names or config dict"
    )
    ...
    remove_duplicates: Optional[DuplicateConfig] = Field(
        default_factory=DuplicateConfig,
        description="Duplicate removal configuration"
    )
    ...
    standardize_dates: Optional[DateConfig] = Field(
        None,
        description="Date standardization configuration"
    )
    standardize_subject_ids: Optional[SubjectIDConfig] = Field(
        None,
        description="Subject ID validation configuration"
    )
    to_numeric: Optional[NumericConfig] = Field(
        None,
        description="Numeric conversion configuration"
    )
    dictionary: Optional[Dict[str, Dict[str, str]]] = Field(
        None,
        description="Dictionary for value replacement by column"
    )
    check_date_sequence: Optional[List[str]] = Field(
        None,
        description="Columns to check for proper date sequence"
    )
    
    # Global settings
    verbose: bool = Field(
        True,
        description="Whether to show progress messages"
    )
    strict_validation: bool = Field(
        False,
        description="Whether to raise errors on validation failures"
    )
    max_memory_usage: Optional[str] = Field(
        None,
        description="Maximum memory usage (e.g., '1GB', '500MB')"
    )
...
class WebConfig(BaseModel):
    ...
    enable_async: bool = Field(
        True,
        description="Enable async processing for large files"
    )
